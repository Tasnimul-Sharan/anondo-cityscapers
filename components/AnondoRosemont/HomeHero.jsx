...
export function HomeHero() {
  return (
    <section className="relative min-h-[calc(100vh-76px)] overflow-hidden bg-[#380a0a] text-white">
      <Image
        src={estateImage}
            ...
            {[
              ...
              <Link
                ...
                className={`group inline-flex items-center justify-center gap-4 px-7 py-4 text-sm font-extrabold uppercase tracking-[0.14em] shadow-2xl shadow-black/25 transition ${
                  item.light
                    ? "bg-white text-[#380a0a] hover:bg-[#380a0a] hover:text-white"
                    : "bg-[#380a0a] text-white hover:bg-white hover:text-[#380a0a]"
                }`}
              >
                {item.label}
      ...
      <div className="absolute inset-x-0 bottom-0 z-20">
        <div className="mx-auto max-w-[1180px] px-5">
          <div className="grid border border-white/15 bg-[#380a0a]/82 backdrop-blur-md sm:grid-cols-2 lg:grid-cols-4">
            {heroStats.map((stat) => (
              <div
                key={stat.label}
