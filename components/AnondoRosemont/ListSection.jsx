    ...
    <section
      className={`group border p-7 md:p-10 ${
        dark
          ...
          : "border-[#380a0a]/15 bg-white/75 text-[#241818] shadow-xl shadow-[#380a0a]/10"
      }`}
    >
      ...
      <h2
        className={`text-2xl font-semibold leading-tight md:text-4xl ${dark ? "text-white" : "text-[#380a0a]"}`}
      >
        {title}
      </h2>
      ...
      {items?.length > 0 && (
        <ul className="mt-6 grid gap-3">
          {items.map((item) => (
            ...
            >
              <span
                className={`mt-[13px] h-1.5 w-1.5 shrink-0 ${dark ? "bg-white" : "bg-[#380a0a]"}`}
              />
              <span>{item}</span>
            </li>
          ))}
