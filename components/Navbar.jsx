  ...
  `}
      ...
      <div className="bg-white border-b">
        <div className="custom-container mx-auto flex justify-between items-center">
          ...
          <Link
            href="/"
            className="flex items-center gap-3 h-28 overflow-hidden"
          >
            <Image
              src="/cityscapers.png"
              alt="Logo"
          ...
          <ul className="hidden lg:flex gap-8 font-semibold relative">
            {menuItems.map((item, i) => (
              <li key={i} className="relative group">
                ...
                {item.submenu && (
                  <ul className="absolute left-0 top-full mt-6 w-72 bg-white/90 backdrop-blur-md shadow-2xl border border-gray-100 rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-4 group-hover:translate-y-0 transition-all duration-500 z-50 overflow-hidden">
                    {item.submenu.map((sub, j) => (
